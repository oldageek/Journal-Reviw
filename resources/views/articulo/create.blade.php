...
@section('content')
    <h2 class="text-center mb-5">Create New Article</h2>
        ...
        <div class="col-md-8">
            <form action="{{ route('articulo.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="titulo" class="form-control @error('titulo') is-invalid @enderror" id="titulo" placeholder="Article Title" value="{{ old('titulo') }}">
                    @error('titulo')
                ...
                <div class="form-group mt-4">
                    ...
                    <select name="status" class="form-control" id="status">
                        ...
                        @foreach ($status as $statu)
                            <option value="{{ $statu -> id }}" {{ old('statu') == $statu -> id ? 'selected' : '' }}>{{ $statu -> status }}</option>
                        @endforeach
                    </select>
